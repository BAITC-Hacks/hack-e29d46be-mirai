...
Нужен Python 3.10+. Интернет не нужен (только для необязательного LLM-ассистента).
...
`run.py` за **~2 секунды** пересчитывает всё от сырых parquet до выгрузок, сам проверяет их
и открывает интерфейс на http://localhost:8000.
...
| `python run.py --data путь/к/data --out путь/к/outputs` | другие папки |
...
оркестратор — `pipeline/main.py`. **Все пороги — в одном файле `pipeline/config.py`.**
...
| `fast_forward_share` | доля исходящей суммы, ушедшая **не позже 2 дней** после какого-либо поступления (по `transactions.parquet`) |
...
| **transit** — транзитный счёт | не seed; есть и вход, и выход; отдаёт дальше **70–130%** полученного **или ≥ 60%** исходящей суммы ушло в течение **2 дней** после поступления (флаг `fast_transit`) | «Отдаёт дальше 81% полученного (1.2 млн ₸ → 1.0 млн ₸); 85% ушло в течение 2 дн. после поступления» |
...
**Итог на данных:** peripheral 1 619 · terminal 281 · transit 248 (из них 202 — сквозной транзит ≤ 2 дней) ·
...
|---|---|
| **Обрыв на 4-м колене**: 444 узла без исходящих — артефакт обхода | Не считаем их конечными получателями автоматически. По умолчанию `peripheral` + флаг `truncated_by_depth` и пометка в evidence. `terminal` — только при ≥ 3 плательщиках и с формулировкой «сток вероятен, не доказан» (таких 3). Узлы колен 1–3 без исходящих — наоборот, настоящие стоки в пределах видимости: обход бы продолжился, будь у них переводы ≥ 5 000 ₸ |
...
pipeline/           метрики, роли, приоритет, кластеры, раскладка, выгрузки; config.py — все пороги
