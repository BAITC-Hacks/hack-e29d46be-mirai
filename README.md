...
При отсутствии ключа или ошибке API предусмотрен локальный ответ. Ключи хранятся только
в локальном `.env`, исключённом из Git. Подробнее: [assistant/README.md](assistant/README.md).
...
| `betweenness` | посредничество на **направленном** графе: как часто узел лежит на кратчайших направленных путях графа; не трассировка конкретных денег |
...
3. Скопируйте полный `gid` выбранного клиента в поиск. Он должен открыться без потери цифр.
...
5. Выберите два связанных клиента для сравнения и проверки направленного пути.
   Если пути нет, это допустимый результат, а не ошибка. Общий получатель также не обязан существовать.
6. Скачайте текущую сцену в SVG, откройте файл в браузере и увеличьте масштаб.
   Сравните узлы, направление стрелок и выделение с картой.
7. Откройте CSV: у каждого клиента должны быть роль, `role_score`, `cluster_id`,
   `priority_score`, числовое `evidence`; для исходной выборки в топ-листе 50 строк.
...
Оценщик ассистента выполняет 12 локальных сценариев с проверкой ссылок на существующие узлы;
это проверка работы ответов, а не измерение точности выявления нарушений.
