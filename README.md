...
| `python run.py --no-serve --data путь/к/data --out путь/к/outputs` | пересчёт в выбранную папку |
| `python run.py --skip-pipeline --out "путь с пробелами/outputs"` | интерфейс на выгрузках из выбранной папки |

Каталог результатов выбирается одинаково для пайплайна и интерфейса:
явный `--out` → переменная окружения `MIRAI_OUTPUT_DIR` → стандартная `outputs/`.
Относительный путь считается от текущей рабочей папки и передаётся серверу как абсолютный.
...
в ролях и в гипотезе кластера. Получилось 91 кластер (61 крупнее 3 узлов, 8 содержат > 1 seed).
