"""Пороги ролей и константы основного пайплайна — в одном месте, чтобы их можно было объяснить жюри.
...
FAST_FORWARD_DAYS = 2                # близость дат исходящих и любого поступления (суммы не сопоставлены)
TRANSIT_FAST_SHARE = 0.6             # ≥ 60% исходящей суммы близки по датам к поступлению
...
EXTRA_FLAGS = {"fast_transit", "in_cycle", "sync_inflow", "terminal_unknown"}
...
METRIC_LABELS = {
    ...
    "betweenness": "Посредничество", "fast_forward_share": "Исходящие в пределах 0–2 дней от поступления, доля",
    "fast_transit_share": "Входящая сумма, сопоставленная с исходящей через 1–2 дня",
    "same_day_transit_share_upper_bound": "Верхняя оценка транзита с учётом того же дня",
    "sync_in_days": "Дней с поступлениями от ≥3 плательщиков",
    "sync_max_payers": "Максимум плательщиков за день",
    "transit_observation_complete_share": "Доля входящих с полным окном наблюдения",
    "repeated_route_count": "Повторяющихся маршрутов через узел",
    "likely_true_terminal": "Гипотеза конечного получателя внутри границы выборки",
    "terminal_unknown": "Конечный получатель не подтверждён",
    "transit_seed_caveat": "Входящие seed видны не полностью",
}
FLAG_LABELS = {
    "fast_transit": "≥80% входящих сопоставлены с исходящими через 1–2 дня",
    "rapid_outflow": "Быстрые исходящие по датам (без сопоставления сумм)",
    ...
    "in_cycle": "Есть направленный цикл в графе",
    "sync_inflow": "Синхронные поступления (≥ 3 плательщика в день)",
    "terminal_unknown": "Сток не подтверждён: граница выборки или неполные входящие seed",
}
